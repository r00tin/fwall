...
getData() {
    ...
    read -p " 请选择（默认aes-128-cfb）" answer
    if [[ -z "$answer" ]]; then
        METHOD="aes-128-cfb"
    else
        case $answer in
...
installNewVer() {
    ...
    if [[ $? -ne 0 ]]; then
        echo
        echo -e " [${RED}错误${PLAIN}]: $OS Shadowsocks-libev 安装失败！ 请反馈"
...
menu() {
    ...
    echo -e "# ${GREEN}作者${PLAIN}: 一指年华                                        #"
    echo -e "# ${GREEN}网址${PLAIN}: https://暂时不告诉你                                    #"
    echo -e "# ${GREEN}论坛${PLAIN}: https://暂时不告诉你                                   #"
    echo "#############################################################"
    echo ""
